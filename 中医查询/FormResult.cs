...
namespace 中医信息管理系统
{
    public partial class FormResult : Form
    {
        ...
        private void SplitAsList()  //拆分输入字符串建立数组
        {
        ...
        private void FormTest_Load(object sender, EventArgs e)
        {
            ...
            if (SearchName == "疾病") //贺
            {
                string sql1 = "select `特效方名称` AS 方药名称,`主治` AS 方药对症,`方药` AS 方药组成 from  (select `特效方名称`,`主治`,`方药` from `秘方特效方` where `主治` like '%" + txt + "%' ";
                string sql2 = sql1 + " or `主治` like '%" + txt + "%'";
                string sql3 = sql2 + "union select `药方名称`,`主治`,`组成` from `偏方药方` where `主治` like '%" + txt + "%'";
                string sql4 = sql3 + " or `主治` like '%" + txt + "%'";
                string sql = sql4 + ") t";
